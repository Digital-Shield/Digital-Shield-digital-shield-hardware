...
static void send_msg_features(uint8_t iface_num,
  ...
  if (device_is_factory_mode()) {
    ...
    MSG_SEND_ASSIGN_VALUE(digital_device_type, DigitalDeviceType_PRO);
    ...
    if (vhdr && hdr) {
      ...
      MSG_SEND_ASSIGN_STRING_LEN(digitalshield_version, ver_str, 5);
    } else {
...
static void detect_installation(vendor_header *current_vhdr,
  ...
  *is_downgrade_wipe = secfalse;
  ...
  if (version_compare(new_hdr->digitshield_version, current_hdr->digitshield_version) <
      0) {
