...
if [[ $RUN_TEST_EMU > 0 ]]; then
  ...
  mv "${TREZOR_PROFILE_DIR}/DigitShield.flash" "${TREZOR_PROFILE_DIR}/DigitShield.flash.bkp" 2>/dev/null
  mv "${TREZOR_PROFILE_DIR}/onkey.sdcard" "${TREZOR_PROFILE_DIR}/DigitShield.sdcard.bkp" 2>/dev/null
