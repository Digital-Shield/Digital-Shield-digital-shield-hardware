...
if TYPE_CHECKING:
    from trezor.messages import EthereumFieldType, EthereumFieldTypeDigitalshield
...
def get_type_name_digitshield(field: EthereumFieldTypeDigitalshield) -> str:
    """Create a string from type definition (like uint256 or bytes16)."""
    from trezor.enums import EthereumDataTypeDigitalshield

    data_type = field.data_type
    size = field.size

    TYPE_TRANSLATION_DICT = {
        EthereumDataTypeDigitalshield.UINT: "uint",
        EthereumDataTypeDigitalshield.INT: "int",
        EthereumDataTypeDigitalshield.BYTES: "bytes",
        EthereumDataTypeDigitalshield.STRING: "string",
        EthereumDataTypeDigitalshield.BOOL: "bool",
        EthereumDataTypeDigitalshield.ADDRESS: "address",
    }

    if data_type == EthereumDataTypeDigitalshield.STRUCT:
        assert field.struct_name is not None  # validate_field_type
        return field.struct_name
    elif data_type == EthereumDataTypeDigitalshield.ARRAY:
        assert field.entry_type is not None  # validate_field_type
        type_name = get_type_name_digitshield(field.entry_type)
        if size is None:
            return f"{type_name}[]"
        else:
            return f"{type_name}[{size}]"
    elif data_type in (EthereumDataTypeDigitalshield.UINT, EthereumDataTypeDigitalshield.INT):
        assert size is not None  # validate_field_type
        return TYPE_TRANSLATION_DICT[data_type] + str(size * 8)
    elif data_type == EthereumDataTypeDigitalshield.BYTES:
        if size:
            return TYPE_TRANSLATION_DICT[data_type] + str(size)
        else:
            return TYPE_TRANSLATION_DICT[data_type]
