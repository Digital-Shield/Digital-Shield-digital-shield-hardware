...
from trezor.enums import EthereumDataTypeDigitalshield as EthereumDataType
from trezor.ui import i18n
from trezor.messages import (
    EthereumFieldTypeDigitalshield as EthereumFieldType,
    EthereumSignTypedDataDigitalshield as EthereumSignTypedData,
    EthereumTypedDataSignatureDigitalshield as EthereumTypedDataSignature,
    EthereumTypedDataStructAckDigitalshield as EthereumTypedDataStructAck,
    EthereumTypedDataStructRequestDigitalshield as EthereumTypedDataStructRequest,
    EthereumTypedDataValueAckDigitalshield as EthereumTypedDataValueAck,
    EthereumTypedDataValueRequestDigitalshield as EthereumTypedDataValueRequest,
)
...
from ..layout import (
    ...
    confirm_typed_value_digitshield,
    should_show_array,
    should_show_struct,
)
...
class TypedDataEnvelope:
    ...
    def encode_type(self, primary_type: str) -> bytes:
        ...
        for type_name in [primary_type] + sorted(deps):
            ...
            fields = ",".join(
                f"{get_type_name_digitshield(m.type)} {m.name}" for m in members
            )
            ...
            if field_type.data_type == EthereumDataType.STRUCT:
                ...
                if show_data:
                    show_array = await should_show_array(
                        ctx=self.ctx,
                        parent_objects=current_parent_objects,
                        data_type=get_type_name_digitshield(entry_type),
                        size=array_size,
                    )
                else:
                    show_array = False
                ...
                for i in range(array_size):
                    ...
                    if entry_type.data_type == EthereumDataType.STRUCT:
                        ...
                        if show_array:
                            await confirm_typed_value_digitshield(
                                ctx=self.ctx,
                                name=field_name,
                                value=value,
                                parent_objects=parent_objects,
