# DigitShield Core Boot Stages

DigitShield T initialization is split into two stages. See [Memory Layout](memory.md) for info about in which sectors each stage is stored.
...
* There is a tool called [headertool.py](https://github.com/ziancube/digitshield-touch-firmware/blob/master/core/tools/headertool.py) which checks validity
  of the bootloader/firmware images including their headers.

## Bootloader Format

DigitShield Core (second stage) bootloader consists of 2 parts:
...
|-------:|-------:|------|-------------|
| 0x0000 | 4      | magic | firmware magic `DSTB` |
...
| 0x03BF | 1      | sigmask | DigitShield signature indexes (bitmap) |
| 0x03C0 | 64     | sig | DigitShield aggregated signature of the bootloader header |

## Firmware Format

DigitShield Core firmware consists of 3 parts:
...
| offset | length | name | description |
|-------:|-------:|------|-------------|
| 0x0000 | 4      | magic | firmware magic `DSTF` |
...
| ?      | 1      | sigmask | DigitShield signature indexes (bitmap) |
| ?      | 64     | sig | DigitShield aggregated signature of the vendor header |
