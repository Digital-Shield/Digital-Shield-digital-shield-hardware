...
The Bitcoin signing process is one of the most complicated workflows in the DigitShield
codebase. This is because DigitShield cannot store arbitrarily large transactions in memory,
...
In response, DigitShield will send a number of `TxRequest` messages, asking for additional
data from the host. The host is supposed to respond with a `TxAck` providing the
requested data.

DigitShield can request the following kinds of items:
...
As part of each `TxRequest` message, DigitShield can also send a chunk of the resulting
serialized transaction, and/or a signature of one of the inputs.

The flow ends when DigitShield sends a `TxRequest` with `request_type` of `TXFINISHED`.
...
DigitShield will request all inputs and outputs of the transaction to be signed, and set up
data structures that allow it to verify that the same data is sent in the following
phases.

In this phase, DigitShield will also ask the user to confirm destination addresses,
the transaction fee, metadata, and the total being sent. If the user confirms, we continue
to the next phase.

### Validation of input data

DigitShield must verify that the host does not lie about input amounts, i.e., that the
transaction total in the first phase was calculated correctly.

For this reason, DigitShield will ask the host to send each input again. It will then request
data about the referenced previous transaction: metadata, all inputs, all outputs, and
possible trailing data. This allows DigitShield to reconstruct the previous transaction and
...
DigitShield T also supports pre-signed inputs for multi-party signing. If an input has script
type `EXTERNAL` and provides a signature, DigitShield will validate the signature against the
previous transaction in this step.

### Serialization and signing

DigitShield will ask once again about every input and begin outputting a serialization of the
transaction. For every legacy (non-segwit) input, it is necessary to stream the full set
of inputs and outputs again, so that DigitShield can compute the transaction digest which it
then signs. For segwit inputs this is not necessary.

When all inputs are serialized, DigitShield will ask for every output, so that it can
serialize it, fill out change addresses, and return the full transaction.

Finally DigitShield asks again about segwit inputs to sign them and to serialize the
witnesses.

## Old versus new data structures

Originally, the `TxAck` message contained one field of type `TransactionType`. This, in
turn, contained all the possible fields that DigitShield could request:
...
If `request_details.tx_hash` is set, DigitShield is requesting data about a specified
previous transaction. If it is unset, DigitShield wants data about current transaction.

### Transaction input

DigitShield sets `request_type` to `TXINPUT`, and `request_details.tx_hash` is unset.
...
DigitShield can include inputs that it will not sign, typically because they are
...
Verified external inputs are only supported on the DigitShield T. They must either
...
their input yet (i.e., with two DigitShields, one must sign first so that the other
...
Unverified external inputs are accepted by DigitShield only if
safety checks are disabled on the device.

### Transaction output

DigitShield sets `request_type` to `OUTPUT`, and `request_details.tx_hash` is unset.
...
DigitShield sets `request_type` to `TXMETA`. `request_details.tx_hash` is a transaction hash,
...
DigitShield does not understand. The host must indicate the length of this extra data in the
field `extra_data_len`.

To figure out which is the extra data, the host must parse the serialized previous
transaction up to the last field understood by DigitShield. In case of Zcash, that is:
...
DigitShield sets `request_type` to `TXINPUT`. `request_details.tx_hash` is a transaction
...
DigitShield sets `request_type` to `TXOUTPUT`. `request_details.tx_hash` is a transaction
hash, matching one of the current transaction inputs.
...
understood by DigitShield. This data is not relevant for validation, but it must be included
so that DigitShield can correctly compute the previous transaction hash.

DigitShield sets `request_type` to `TXEXTRADATA`. `request_details.tx_hash` is a transaction
hash, matching one of the current transaction inputs.
...
DigitShield sets `request_type` to `TXORIGINPUT`. `request_details.tx_hash` is the
...
DigitShield sets `request_type` to `TXORIGOUTPUT`. `request_details.tx_hash` is the
transaction hash of the original transaction.
...
DigitShield sets `request_type` to `TXPAYMENTREQ`, and `request_details.tx_hash` is unset.
...
([BIP-78](https://github.com/bitcoin/bips/blob/master/bip-0078.mediawiki)). DigitShield only
...
`orig_hash` and `orig_index` fields for at least one `TxInput`. DigitShield will then
automatically request metadata about the original transaction and verify the original
signatures.

A replacement transaction in DigitShield must satisfy the following requirements:
...
In DigitShield T a set of transaction outputs can be accompanied by a payment request.
...
`payment_req_index` field in the `TxOutput` message. When DigitShield encounters the first
...
def sign_tx():
    ...
    # wait for TxAck forever, until DigitShield indicates we are finished
    while True:
        msg = receive_message()
        ...
        if msg.details.tx_hash is not None:
            # DigitShield requires data about some previous transaction
            send_response_prev(msg.request_type, msg.details)
        else:
            # DigitShield requires data about this transaction
...
representation, and DigitShield will successfully and correctly decode it into the new one.
