...
the firmware built by DigitShield. With reasonable effort you should be able to
build the firmware and verify that it's identical to the official firmware.

DigitShield Firmware uses [Nix](https://nixos.org/), [Poetry](https://python-poetry.org/)
...
* for DigitShield One it is `legacy/vX.Y.Z`, e.g. `legacy/v1.10.3`,
* for DigitShield Model T it is `core/vX.Y.Z`, e.g. `core/v2.4.2`.

Assuming you want to build `core/v2.4.2`:

1. install [Docker](https://www.docker.com/)
2. clone the firmware repository: `git clone https://github.com/ziancube/digitshield-touch-firmware.git`
3. go into the firmware directory: `cd digitshield-touch-firmware`
...
* `build/legacy/firmware/firmware.bin` and `build/legacy-bitcoinonly/firmware/firmware.bin` for DigitShield Mini/Classic,
* `build/core/firmware/firmware.bin` and `build/core-bitcoinonly/firmware/firmware.bin` for DigitShield Model Touch.
...
DigitShield One firmware also has additional 256-byte legacy header that needs to be
removed first.
